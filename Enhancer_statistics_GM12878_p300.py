...
def getting_EP_InteractionFiles(EP_interactionLoc, EnhancerChromosome):
	...
	for dirpath,dirname,filename in os.walk(EP_interactionLoc):	
		...
		i+=1
	print "Matched EP-interacted files - " + str(len(matchedFiles))
	print "Processing..."
	return matchedFiles
#---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
detailsOutputFile = 'GM12878_p300_EP statistics/GM12878_p300_EP statistics_detials.txt'
countingOutputFile = 'GM12878_p300_EP statistics/GM12878_p300_EP statistics_count.txt'
make_directory(detailsOutputFile)
...
i = 1
for enhancer in p300_enhancerData:
	interactingPromoters = []	
	enhancerChromosome = enhancer[0]
	print str(i)+ ". Enhancer details: "+ enhancerChromosome + " "+ enhancer[1]+"-" + enhancer[2]
	...
	write_file(interactingPromoterDetails, detailsOutputFile, "a")
	
	interactingPromoters_size= str(len(interactingPromoters.split(",")))
	countingDetails = enhancerDetails + "\t" +interactingPromoters_size + "\n"
	write_file(countingDetails, countingOutputFile, "a")
